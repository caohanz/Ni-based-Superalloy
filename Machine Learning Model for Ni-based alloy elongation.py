...
# Set it to 1, the recursive feature elimination is going to remove one feature through each iteration, step corresponds to the (integer) number of features to remove at each iteration.
    selector.fit(X_train, y_train['EL/%'].values.reshape(-1, 1))
...
# Set up a training model to test the evaluation, using SVR which is used to find a vector platform where has the shortest distance between all the features in a set
    ...
    regressor.fit(X_train,y_train['EL/%'].values.reshape(-1, 1))
    X_test = scaler.fit_transform(X_test)
    y_pred1 = regressor.predict(X_test)
    
    from sklearn.metrics import r2_score  # Evaluate the number of  using r2 score
    r2_a = r2_score(y_pred1, y_test['EL/%'])
    print(r2_a)
    empty.append(r2_a)
empty = empty[::-1]
print(max(empty), (empty.index(max(empty))*1)+6)  # Find the largest r2 score and how many elements are in there
best_selected_features = 'test_T,Ni (wt.%),Co (wt.%),Mo (wt.%),C (wt.%),Ta (wt.%),Zr (wt.%),W (wt.%),B (wt.%),Si (wt.%),Mn (wt.%),solution_step_1_T/℃,cooling_rate/℃/s,aging_step2_t/h,solution_step_1_t/h,solution_step_2_T/℃,solution_step_2_t/h,aging_step1_T/℃,aging_step1_t/h,aging_step2_T/℃'.split(",")

# Ni has been mannually fitlered because it acts as a base-elements in the alloy

best_selected_features = 'test_T,Co (wt.%),Mo (wt.%),C (wt.%),Ta (wt.%),Zr (wt.%),W (wt.%),B (wt.%),Si (wt.%),Mn (wt.%),solution_step_1_T/℃,cooling_rate/℃/s,aging_step2_t/h,solution_step_1_t/h,solution_step_2_T/℃,solution_step_2_t/h,aging_step1_T/℃,aging_step1_t/h,aging_step2_T/℃'.split(",")
# Hence the number of features selected is 19, which has a coreesponding r2 score of 0.5680353771629827
best_selected_features = df[best_selected_features]



# Evaluate the model using cross validation set
X_train, X_test, y_train, y_test = train_test_split( best_selected_features , targets, test_size=0.33, random_state=60) 
...
rf_random.fit(X_train, y_train['EL/%'])
a = rf_random.best_params_ # Selected hyper parameters
# The selected hyper parameters are following: {'n_estimators': 1000, 'min_samples_split': 2, 'min_samples_leaf': 1, 'max_features': 'sqrt', 'max_depth': 20, 'bootstrap': True}
...
random_search.fit(X_train, y_train['EL/%'])
b = random_search.best_params_ # Selected hyper parameters 
# The selected hyper parameters are following: {'n_estimators': 200, 'min_samples_split': 10, 'min_samples_leaf': 4, 'max_depth': 6, 'learning_rate': 0.01}
...
tree_search.fit(X_train, y_train['EL/%'])
c = tree_search.best_params_ # Selected hyper parameters 
# The selected hyper parameters are following: {'min_samples_split': 9, 'min_samples_leaf': 7, 'max_features': 'sqrt', 'max_depth': 5}
...
krr_search.fit(X_train, y_train['EL/%'])
d = krr_search.best_params_ # Selected hyper parameters 
# The selected hyper parameters are following: {'kernel': 'linear', 'gamma': 100.0, 'alpha': 100.0}
...
model_rf = RandomForestRegressor(n_estimators=1000,max_depth=20,min_samples_split=2,min_samples_leaf=1,max_features='sqrt',bootstrap=True)
model_rf = clone(model_rf)
model_gb = GradientBoostingRegressor(learning_rate=0.01, n_estimators=200,min_samples_split=10, min_samples_leaf=4,max_depth=6)
model_gb = clone(model_gb)
model_tree = DecisionTreeRegressor(max_depth=5, min_samples_split=9, min_samples_leaf=7,max_features='sqrt')
model_tree = clone(model_tree)
model_krr = KernelRidge(alpha=100.0,kernel='linear', gamma=100.0)
...
for i in model:
    value = cross_val_score(i,X_train,y_train['EL/%'], scoring = 'r2', cv=kf) 
    value = value.mean()
    value = round(value,3)
...
for i in model:
    value = cross_val_score(i, X_train,y_train['EL/%'], scoring = 'neg_mean_squared_error', cv=kf) 
    value = value.mean()
...
imp = []
for i in model:
    i.fit(X_train,y_train['EL/%'])
    y_pred = i.predict(X_test)
    r2_value = mean_squared_error(y_pred,y_test['EL/%'])
    imp.append(r2_value)
...
width = 0.35
fig,ax = plt.subplots()
bar1 = ax.bar(x- width * 0.5,  mean_squared_error_train, width, label = 'Mean Squared Value for training set')
bar2 = ax.bar(x + width * 0.5,  imp, width, label = 'Mean Squared Value for test set')
ax.set_xticks(x)
...
plt.title('Mean Squared Value evaluation for selected models')
plt.show()

# By r2 score and mean squared error, random forest regressor has been selected
# Apply grid search for Random Forest regressor based on its random search result
from sklearn.model_selection import GridSearchCV
# Create the parameter grid based on the results of random search 

# The selected hyper parameters are following: {'n_estimators': 1000, 'min_samples_split': 2, 'min_samples_leaf': 1, 'max_features': 'sqrt', 'max_depth': 20, 'bootstrap': True}
boost_grid = param_grid = {
    'n_estimators': np.arange(100,301),
    'max_depth': [None, np.arange(1,31)],
    'min_samples_split': np.arange(2, 11),
    'min_samples_leaf': [1, 2, 4],
                                   # Try additional values
    'bootstrap': [True]
}

gbr = RandomForestRegressor()
grid_search = GridSearchCV(gbr, param_grid = boost_grid, cv=5, n_jobs=-1)
grid_search.fit(X_train, y_train['EL/%'])
parameters = grid_search.best_params_
print(parameters)
#  The selected hyper parameters are following: {'n_estimators': 275, 'min_samples_split': 2, 'min_samples_leaf': 1,  'max_depth': None, 'bootstrap': True}
...
gbr = RandomForestRegressor(n_estimators=275,max_depth=None,min_samples_split=2,min_samples_leaf=1,bootstrap=True)
gbr.fit(X_train,y_train['EL/%'])
y_predict = gbr.predict(X_test)
r2_value = r2_score(y_predict,y_test['EL/%'])
print(r2_value)
...
validation_value = validation_value['EL/%'].values
